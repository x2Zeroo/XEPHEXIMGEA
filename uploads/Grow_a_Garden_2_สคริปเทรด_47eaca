...
--
-- คูลดาวน์จริงของเกมอยู่ที่ ~3-5 วิต่อครั้ง (ยืนยันจากผู้ใช้) จึงตั้ง RETRY_WAIT
-- ไว้ 5 วิ x 12 ครั้ง (รอสูงสุด ~60 วิต่อ chunk ก่อนยอมแพ้จริง) ค่าเดิม 0.1 วิ
-- สั้นเกินไปมาก ทำให้ retry ครบ 10 ครั้งภายใน ~1 วิ ทั้งที่คูลดาวน์ยังไม่คลาย
-- เลย mark "ค้าง" ทั้งที่จริงแค่ต้องรอ
--
...
local function SendGiftChunked(targetUserId, category, itemKey, totalCount, note)
    ...
    while remaining > 0 do
        ...
        while chunkSentSoFar < chunkTarget do
            ...
            local pollElapsed = 0
            while pollElapsed < RETRY_WAIT do
                task.wait(POLL_INTERVAL)
                pollElapsed = pollElapsed + POLL_INTERVAL
                afterCount = GetAvailableCount(category, itemKey)
                if afterCount <= beforeCount - needNow then break end
            end
...
local function PollCommands()
    while true do
        ...
        for i, item in ipairs(items) do
            if i > 1 then task.wait(RETRY_WAIT) end
            ...
            if result.Status ~= "sent" then
                local statusLabel = (result.Status == "partial" and "บางส่วน") or "ไม่สำเร็จ"
