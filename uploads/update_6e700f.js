...
// - ต่อให้ secret ถูก แต่คีย์ (licenseKey) ผิด/ไม่มีในฐานข้อมูล = ไม่เห็นอะไรเลย
// - API นี้แก้ไขได้แค่ hwid อย่างเดียว
//
...
module.exports = async function handler(req, res) {
    ...
    try {
        ...
        console.error("update.js crashed:", err);
        try {
...
async function handleRequest(req, res) {
    // ต้องเป็น Node 18+ ถึงจะมี fetch
    if (typeof fetch !== "function") {
        res.status(500).json({ error: "Server misconfigured: Node runtime too old, need Node 18+" });
        return;
    }
