...
// ===== เช็ค @snazzah/davey (จำเป็นสำหรับ DAVE protocol ของ Discord voice) =====
// Discord เปลี่ยนมาใช้ voice gateway v8 ที่บังคับใช้ DAVE (E2E encryption)
// ถ้าไม่มีแพ็กเกจนี้ การต่อห้องเสียงจะค้างที่ connecting -> signalling แล้ว timeout
// (นี่คือสาเหตุของ error เดิม ไม่ใช่โฮสต์บล็อก UDP!)
try {
  require("@snazzah/davey")
  console.log("🔐 โหลด @snazzah/davey สำเร็จ (DAVE protocol พร้อมใช้งาน)")
} catch {
  console.error(
    "❌ ไม่พบ @snazzah/davey! การต่อห้องเสียงจะล้มเหลว กรุณาลบโฟลเดอร์ node_modules แล้วรันใหม่เพื่อติดตั้ง dependencies ให้ครบ",
  )
}
...
// ถ้าต่อไม่ติด จะ destroy แล้วสร้าง connection ใหม่ทั้งอัน (เสถียรกว่า rejoin)
async function connectToVoice(voiceChannel, guild) {
  ...
  for (let attempt = 1; attempt <= MAX_TRIES; attempt++) {
    ...
    // log สถานะ + error แบบเต็มๆ ไว้ดูใน console เวลามีปัญหา
    if (!connection._v0LogsAttached) {
      ...
      connection.on("error", (err) => {
        console.error("Voice connection error:", err?.stack || err?.message || err)
      })
    }
    ...
    try {
      ...
      // รอสักครู่ก่อนลองใหม่ ให้ Discord เคลียร์ session เก่าก่อน
        ...
        if (!q) {
          ...
          if (!connection) {
            ...
            return interaction.editReply(
              "❌ เชื่อมต่อห้องเสียงไม่สำเร็จ (ลองครบ 3 ครั้งแล้ว) ดู error ใน console เพื่อหาสาเหตุ",
            )
          }
