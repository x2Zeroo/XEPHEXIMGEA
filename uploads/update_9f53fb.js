...
async function findKey(licenseKey) {
    ...
    if (!/^[A-Za-z0-9]{1,64}$/.test(licenseKey)) return null;
    const rows = await sb("GET", `keys?key=eq.${encodeURIComponent(licenseKey)}&select=*`);
        ...
        switch (action) {
            ...
            case "verify_license": {
                ...
                if (expired) {
                    ...
                    if (row.status !== "expired") {
                        await sb("PATCH", `keys?key=eq.${encodeURIComponent(licenseKey)}`,
            ...
            case "update_hwid": {
                ...
                const updated = await sb("PATCH",
                    `keys?key=eq.${encodeURIComponent(licenseKey)}`,
                    { hwid },
